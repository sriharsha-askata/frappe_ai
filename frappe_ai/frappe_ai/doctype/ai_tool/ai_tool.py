...
"""`AI Tool` — ported from `flow`'s `Flow Tool` (see `apps/flow/flow/flow/doctype/flow_tool/flow_tool.py`).

Execution is the load-bearing difference from `flow`: `to_tool()` here resolves through
`frappe_ai.lib.resolver.resolve_tool`, which runs Script tools through
`frappe_ai.utils.safe_exec` (the hardened namespace built in Phase 1), not the broader
`frappe.utils.safe_exec` `flow` uses. See ADR 0006.
"""
...
class AITool(Document):
	...
	def to_tool(self):
		"""Resolve this row into a runtime `Tool` the Agno agent can call.

		Returns:
			frappe_ai.lib.tool.Tool: The resolved, callable tool.
		"""
		from frappe_ai.lib.resolver import resolve_tool
	...
	def _validate_type_fields(self):
		if self.type == "Imported":
			...
			if not IMPORT_PATH_PATTERN.match(self.import_path):
				frappe.throw(
					_(
						"Import Path must be a dotted Python path (e.g. <code>frappe_ai.tools.builtins.read</code>)."
					),
